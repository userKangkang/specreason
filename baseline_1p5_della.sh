...
THRESHOLD=9
NUM_REPEATS=2
BASE_MODEL_NAME="deepseek-ai/DeepSeek-R1-Distill-Qwen-1.5B"  # "Qwen/QwQ-32B" or "deepseek-ai/DeepSeek-R1-Distill-Llama-70B" or "NovaSky-AI/Sky-T1-32B-Preview"
BASE_MODEL_ABBRV="deepseek-1.5B"  # "Qwen-32B" or "deepseek-70B" or "NovaSky-Preview"
...
for id in "${ids1[@]}"; do
    ...
    for repeat_id in $(seq 0 $((NUM_REPEATS - 1))); do
        python baseline_1p5.py --dataset_name "$DATASET_NAME" --problem_id "$id" --repeat_id "$repeat_id" --score_threshold "${THRESHOLD}" --score_method "${JUDGE_SCHEME}" --token_budget "$TOKEN_BUDGET" --output_dir "$OUTPUT_DIR" >> "$logfile" 2>&1 &
