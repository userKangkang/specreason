...
OUTPUT_DIR="${DATA_DIR}/specreason/results_cot/${JUDGE_SCHEME}_${THRESHOLD}/${DATASET_NAME}/${BASE_MODEL_ABBRV}_${SMALL_MODEL_ABBRV}"
...
for id in "${ids1[@]}"; do
    ...
    for repeat_id in $(seq 0 $((NUM_REPEATS - 1))); do
        python spec_cot.py --dataset_name "$DATASET_NAME" --problem_id "$id" --repeat_id "$repeat_id" --score_threshold "${THRESHOLD}" --score_method "${JUDGE_SCHEME}" --token_budget "$TOKEN_BUDGET" --output_dir "$OUTPUT_DIR" >> "$logfile" 2>&1 &
